                        ...
                        <div class="card-body">
                            ...
                            <div class="bs-stepper-content">
                                <form action="{{ route('student-assessment.store') }}" method="post" enctype="multipart/form-data">
                                    @csrf
                                    <div class="col-12">
                                        <label class="form-label">Course: {{ $assessment->course->title }}</label>
                                        <input class="hidden" type="text" name="assessment_id" value="{{ $assessment->id }}" hidden>
                                    </div>
                                    @if(isset($assessment->link) && file_exists($assessment->link))
                                    <div class="col-12">
                                        <label class="form-label">A Assessment File Found</label>
                                        <a href="{{ asset($assessment->link) }}">Click to Open</a>
                                    </div>
                                    @endif
                                    <div class="col-12">
